"""
EXPERIMENT B: Lean Feature Set (drop redundant, add high-value)
================================================================
Removes redundant features, adds 4 high-value features identified by research:

  DROPPED (8 features):
    - peak_amp_L0, peak_amp_L1 (redundant with area + template)
    - polarity_L0, polarity_L1 (subsumed by template correlation)
    - rel_peak_L0, rel_peak_L1 (redundant with rel_area)
    - rel_width_L0, rel_width_L1 (lower importance)

  ADDED (4 features):
    - rr_asymmetry = pre_rr / post_rr (flags premature beats)
    - compensatory_ratio = (pre+post)/(2*avg) (V vs S separator)
    - max_slope = max|dV/dt| per lead (ventricular beat detector)
    - rr_std_10 = running std of last 10 RR (episode detector)

  RESULT: 15 features from ~11 circuits, ~100 nW

  Includes ALL training improvements from Exp A.

Run: python experiments/exp_b_lean_features.py --split inter
"""
...
parser = argparse.ArgumentParser(description='Exp B: Lean features')
...
print("  Lean features: 16 features from ~11 circuits")
...
# ================================================================
# DATA LOADERS + MODEL (same as Exp A)
# ================================================================
...
label = f"expB_h{hidden}_s{num_steps}"
...
frontend_nJ = 5 * 0.833 + 95 * 0.100  # ~11 circuits, ~100 nW
